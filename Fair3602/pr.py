...
val_name = "pr_{}_{}_multi30.txt".format(clf_name,dataset_used)
...
for r in range(20,50):
    ...
    prejudice = PrejudiceRemover(sensitive_attr=macro_var[dataset_used], eta=25)
    prejudice = prejudice.fit(dataset_orig_train)
    pred_prejudice = prejudice.predict(dataset_orig_test)

    round_result = measure_final_score(dataset_orig_test, pred_prejudice,macro_var[dataset_used])
